...
#  define WHIO_DEBUG if(WHIO_DEBUG_ENABLED) printf("WHIO_DEBUG: %s:%d:%s():\t",__FILE__,__LINE__,__func__); if(WHIO_DEBUG_ENABLED) printf
#else
...
The size of whio_off_t is determined by the WHIO_SIZE_T_BITS parameter,
and is always (except in 64-bit mode) one step larger than whio_size_t.
e.g. 32-bit whio_size_t has a 64-bit whio_off_t. This is to allow the
full range of whio_size_t to be used as an offset. If they had the same
bitness, whio_off_t could only cover half of the whio_size_t range.
*/
...
extern "C" {
    ...
    */
    ...
    WHIO_MODE_WRITE = 0x02,
    /** Mask for "only" modes (r/o, w/o). */
    WHIO_MODE_FLAG_ONLY = 0x04,
...
/**
   ...
   Returns a positive number of mode appears to be writeable, 0 if it
   appears to be read-only, and a negative value if it cannot
   determine the mode. Note that the whio_dev API does not
   specifically support a write-only mode, and a mode string of "w" is
   equivalent to "w+" or "r+" for purposes of this function.
...
/**
   ...
   - [NOT TRUE ANYMORE:] locking is always done by absolute device
   coordinates, rather than via interpreting relative to SEEK_SET,
   SEEK_CUR, or SEEK_END. This simplifies the implementations and
   removes a number of possibilities for logic errors in the
   device-positioning calculations. The book "Advanced Programming in
   the Unix Environment" (APUE) explains (i later read) that SEEK_CUR
   and SEEK_END are necessary (for proper locking call ordering) if
   the application does not know the device's current
   position. However, in my use cases involving locks i've always
   known the position in advance (fixed-sized records at a known file
   offset), so i'd never come across this problem. If it becomes a
   problem, a whence argument _might_ be added to this class.
...
/**
   ...
   then dev must not be read-only. Normally (dev->api->iomode(dev)>0)
   signifies write mode, but this implementation also treats
   (iomode<0) as writable because (iomode<0) represents "undetermined
   mode", and some devices really do not know whether they are
   writable or not (e.g. those returned by whio_dev_for_FILE()). If
   (iomode==0) and a write lock is requested, whio_rc.AccessError is
   returned.
...
/**
   ...
   - The iomode() member will initially return a negative value
   ...
   writes then the i/o mode will be adjusted to reflect that it is
   writable.
...
/**
   ...
   - flags is a bitmask of flags specifying how to open the file
   (read/write, etc.). It must be a mask of values from the
   ...
   *tgt. This function tries to map the errno values reported by
   ...
   @see whio_dev_for_fileno()
*/
int whio_dev_posix_open2( whio_dev ** tgt, char const * fname, enum whio_iomodes flags, short filePerms );
...
/**
   ...
   *can* be expanded by manually calling truncate() on them.
   ...
   - iomode() always returns a positive value.
*/
...
/**
   ...
   whio_dev_ioctl_BUFFER_uchar_ptr without violating the constness of
   read-only buffers.
...
/**
   ...
   return values for my write() calls.
...
/**
   ...
   seem to be useful enough to warrant the effort. Another option
   would be to expand/shrink the subdevices' defined bounds upon
   truncating, but that could cause some unwanted side-effects in
   client code and would require that this class have a more intimate
   relationship with the parent device (at least in the case of
   growing a subdevice, as that might require growing the parent).
...
/**
   ...
   @code
   size_t count = 0;
   size_t rc = whio_encode_pack( mybuffer, &count,
...
#line 8 "include/wh/whio/whio_epfs_config.h"
...
"stable". Seems to work so far, though.
*/
#define WHIO_EPFS_CONFIG_ENABLE_MEMPOOL 1
...
#line 8 "include/wh/whio/whio_epfs.h"
...
extern "C" {
    ...
    */
    ...
    /**
       Old flag - can be recycled.
     */
    WHIO_EPFS_FLAG_CAN_BE_RECYCLED = 0x01,
    ...
    */
    ...
    /** Used by pfs objects. We can do away with this,
        using the whio_epfs_iomodes enum instead.
     */
    ...
    */
    ...
    /**
       I/O mode flags for use when opening pseudofiles.
       TODO: also support these for opening EFS files
       by name.
...
#define whio_epfs_block_list_empty_m {NULL,0,0}
    ...
    /**
       ...
       The 'link' member gets set when a given inode is opened
       multiple times. If the 'link' member of a handle points to a
       different handle, then the linked-to inode is used for all
       internal operations regarding the inode. This is so that
       multiple openers all share the same basic inode data, but they
       each still have their own position cursor into the data bytes.
       This requires some special care in how we internally finagle
       handles, however, to avoid updating the wrong inode or to avoid
       prematurly deallocating the shared inode instance. Internally,
       when a handle is linked, we must be careful not to use its
       inode member (and it will be initialized to empty values), but
       the inode of the origin handle (as it is called throughout the
       docs) instead.

       TODO:

       Make the inode member a pointer, and allocate it separately for
       the first handle to open it. This will allow us to remove the
       special-case origin inode tracking altogether, will simplify
       certain code, and make this class a few bytes smaller.
    */
    struct whio_epfs_handle
    {
        /**
            ...
            If the 'link' member is set to point to a different handle
            then this inode object must not be used - use the linked
            one instead.

            TODO: make this a pointer and allocate it along with the
            containing handle object in cases where it is the first
            opening. That will save (sizeof(whio_epfs_inode) -
            sizeof(whio_epfs_inode*)) bytes and avoid a lot of the
            fudgery we have to do to make sure we get the real inode
            object for multi-opened handles.
        */
        ...
        */
        ...
        /**
           The number of current active open()s for this handle.

           This will always be 1 for handles which link to another,
           and will be greater than 1 for handles to which other
           handles link. It will always be 1 for origin handles to
           which no other handle links.

           The handle-close mechanism frees a handle when its
           openCount drops to 0.
        */
        uint8_t openCount;
        ...
        */
        ...
        /** Links to a proxy object, for multi-opened inodes
            so we can share the underlying inode without
            sharing, e.g. the r/w-mode used for a given instance.
        */
        struct whio_epfs_handle * link;

        /** The whio_epfs this object came from. */
        struct whio_epfs * fs;
        /**
           An internal stamp which we mark all dynamically-allocated
           whio_epfs_handle objects with. Not persistent. This value
           is used in whio_epfs_handle_free() to determine whether the
           object should really be freed or not (as a kludge to help
           support transparent cleanup of stack- vs. heap-allocated
           objects).
        */
    ...
    */
    struct whio_epfs_fsopt
    {
        ...
        */
        ...
        /** Maximum number of blocks a EFS can hold. 0 means a limit
            ...
            add blocks to a block-capped EFS, by bumping up the
            number. The next-free-block routine will create blocks as
        ...
        */
        ...
        /**
           The size of each data block in a EFS container.
        */
...
#  error "WHIO_EPFS_ID_T_BITS must be one of: 8, 16, 32, 64"
    ...
    */
    whio_epfs_sizeof_magic = 1/*tag byte*/
        + (whio_sizeof_encoded_uint16 * 6/*whio_epfs_magic_bytes[0..4]*/)
        ,
    /**
    ...
    */
    struct whio_epfs_handle_list
    {
        ...
        Maintenance note: this list is a (**) instead of a (*), unlike
        the other list types this library uses, because of how handle
        linking works.
        */
...
#define whio_epfs_handle_list_empty_m {        \
    ...
    */
    struct whio_epfs_allocator
    {
        /**
           Must behave semantically like realloc(3) except that the
           data parameter may hold any allocator state.
        */
        void * (*realloc)( void * m, size_t n, void * data );
        /**
           The data passed as the third argument to the realloc()
           member.
        */
        void * data;
    };
    typedef struct whio_epfs_allocator whio_epfs_allocator;
#define whio_epfs_allocator_empty_m { NULL/*realloc*/,NULL/*data*/ }
    extern const whio_epfs_allocator whio_epfs_allocator_stdalloc;
    ...
    */
    struct whio_epfs_namer_api
    {
        /**
           ...
           inode #i. If name is NULL or len is 0 then the inode should
           be marked as is free/unused.
           ...
           - whio_rc.RangeError: another inode already has the same
           name.
        */
        int (*set)( struct whio_epfs_namer * self, whio_epfs_id_t i, char const * name, whio_size_t len );

        /**
           Gets the name for the given inode ID and copies it to name
           (which must be at least len bytes long, including the
           trailing null). If no name is set, strlen(name) will be 0
           after returning from this function.

           Returns non-0 on error. Error conditions include:

           - whio_rc.ArgError: Arguments are invalid, including !self
           or !name.

           - whio_rc.RangeError: len is too short to store the whole
           name plus the null terminator. i is out of range.
        */
        int (*get)( struct whio_epfs_namer * self, whio_epfs_id_t i, char * name, whio_size_t len );

        /**
           Searches for the given null-terminated name. If found, i is
           assigned its inode value. Failing to find an entry is _not_
           an error (0 is returned), but i will be assigned to 0
           (which is not a valid inode ID).
           ...
           - whio_rc.RangeError: i is out of range or name is too long
           for this namer object.
        */
        int (*search)( struct whio_epfs_namer * self, char const *name, whio_epfs_id_t * i );

        /**
           For each "used" entry in the names table, it must call
           callback( self, entryID, entryName, nameLengthInBytes,
           callbackData ). If callback() returns non-0 then looping
           must stop and return that error code. The order of the
           calls is implementation-dependent, and need not be in
           sequential inode order, but the callback must only be
           called once per entry and only for in-use entries.
        */
        int (*foreach)( struct whio_epfs_namer * self,
                        int (*callback)( struct whio_epfs_namer *, whio_epfs_id_t id, char const * name, whio_size_t length, void * callbackData ),
                        void * callbackData );
        ...
        /**
           ...
           to clean it up or re-initialize it for further use.
    ...
    /**
       ...
       whio_epfs_namer is an attempt at a backend-generic interface
    ...
    struct whio_epfs_namer
    {
        ...
        */
        ...
        /**
           ...
           private API of this instance. Any dynamically-allocated
           resources stored here should be cleaned up when the
           api->cleanup() member is called.
        */
        whio_impl_data impl;
    };
    /** Convenience typedef. */
    ...
    struct whio_epfs
    {
        ...
        */
        ...
        /**
           ...
           (if client.dtor is not null) when the EFS is closed.
        */
        whio_client_data client;
        /**
        ...
        struct whio_epfs_hints
        {
            ...
            */
            ...
            /**
               ...
               The library does not yet lock everything it needs to - only
               the skeleton for that support is in place.
        ...
        */
        ...
        /**
           ...
           memory. Will someday completely replace the pool member.
        */
...
#define whio_epfs_empty_m {                      \
    ...
    */
    ...
    /**
       ...
       null data argument doesn't make terribly much sense, but it's
       not stricly illegal as long as the dtor function gracefully
       handles being passed NULL.

       If client data had previously been applied for fs, it is
       replace and it is not destroyed, even if it had a destructor
...
#define whio_epfs_setup_opt_empty_m {\
    ...
    */
    ...
    /**
       ...
       @code
       whio_dev * dev = whio_dev_for_filename( "my.epfs", "r+" );
    ...
    */
    ...
    /**
       ...
       local file (local=accessible via a filesystem path). If the file
       exists and allowOverwrite is false then whio_rc.AccessError
       is returned, otherwise the file is created or overwritten
    ...
    */
    ...
    /**
       ...
       but internals which muck with persistent
    ...
    */
    ...
    /**
       ...
       *dev will be set to null when this function returns. If the
       ...
       - flush() will update the inode table with the object's current
       inode metadata (e.g. size, timestamp, and first block
       ID). Closing the device will flush it if it is opened in write
       mode.
       ...
       by others are closed as normal, but their resources will not
       actually be freed until all handles linked to them are also
       ...
       problematic, so this will likely eventually be fixed.)
    */
    int whio_epfs_dev_open( whio_epfs * fs, whio_dev ** dev, whio_epfs_id_t inodeID, enum whio_iomodes mode );
...
#line 8 "include/wh/whio/whio_udb.h"
...
#define WHIO_UDB_ENABLE_METRICS 1
    ...
    */
    ...
    /**
       ...
       less than, 0, or greater than 0 to signify that lhs is less
...
#define whio_udb_hints_empty_m {0/*nextFree*/,0/*usedList*/,0/*highestID*/}
    ...
    /**
       Intrnally used for tracking whio_udb statistics. If this code
...
#define whio_udb_metrics_empty_m { 0/*placeholder*/ }
    ...
    */
    ...
    + whio_sizeof_encoded_uint32 /* hash */
    ...
    */
    ...
    {
        /**
           whio_dev::iomode()-complient value. Will be the most
           same as dev->api->iomode(dev).
        */
        short iomode;
        /**
           Not yet used.
...
#define whio_udb_empty_m {\
    ...
    */
    ...
    /**
       ...
       "constructed" sets, and this function always prefer a
    ...
    */
    ...
    /**
       ...
       If db or key are non-NULL then they MUST be able to be legally
    ...
    */
    ...
    /**
       ...
       success.  Note that not finding a record is treates as an error
    ...
    */
    ...
    /**
       Equivalent to whio_udb_foreach_matching_likes(db,&like,caseSensitive,callback,callbackData).
    */
    ...
    */
    ...
    /**
       ...
       The returned memory is valid until the record is destroyed or
       until it is passed as the final argument to whio_udb_search()
       (or a similar function which recycles a record).
...
#line 8 "include/wh/whalloc_amalgamation.h"
...
extern "C" {
   ...
   */
#if defined(WHALLOCX_BITNESS_OVERRIDE)
#    define WHALLOC_BITNESS (WHALLOCX_BITNESS_OVERRIDE)
#else
#    define WHALLOCX_BITNESS_OVERRIDE 0 /* only to keep doxygen happy. */
...
#   define WHALLOC_MASK ((uint64_t)0xffffffffffffff)
