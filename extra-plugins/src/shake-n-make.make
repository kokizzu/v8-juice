...
ShakeNMake.MAKEFILE = $(lastword $(MAKEFILE_LIST))#$(PWD)/shake-n-make.make
...
%.o: %.cpp $(ShakeNMake.MAKEFILE) $(PACKAGE.MAKEFILE)
	@$(call ShakeNMake.CALL.SETX,"CXX [$@] ..."); \
	 $(CXX) $(CXXFLAGS) $($(*).OBJ.CXXFLAGS) \
		...
		$(CPPFLAGS) $($(*).OBJ.CPPFLAGS) \
		-c -o $@ $<
%.o: %.cc $(ShakeNMake.MAKEFILE) $(PACKAGE.MAKEFILE)
	@$(call ShakeNMake.CALL.SETX,"CXX [$@] ..."); \
	 $(CXX) $(CXXFLAGS) $($(*).OBJ.CXXFLAGS) \
		$($(*).OBJ.INCLUDES) \
		$(CPPFLAGS) $($(*).OBJ.CPPFLAGS) \
...
#
# Note that we have to pass both CFLAGS and CXXFLAGS because .SOURCES might
...
#   LDFLAGS, $(1).DLL.LDFLAGS, -shared -export-dynamic
#   $(1).DLL.CPPFLAGS
#
...
INSTALL.DOXYGEN = $(ShakeNMake.DOXYGEN.OUTPUT_DIR.HTML)
